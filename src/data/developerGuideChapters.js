  ...
  // =========================================================================
  {
    ...
    sections: [
      ...
      },
      {
        ...
        paragraphs: [
          'Every date/time field goes through `components/ui/DateField.jsx`, a thin wrapper over the `react-datepicker` npm package — there is no browser-native `<input type="date">` or `datetime-local` left anywhere. It keeps the exact string contract the native inputs had — `YYYY-MM-DD`, or `YYYY-MM-DDTHH:mm` with `withTime`, and `\'\'` when empty (same for `min`/`max`) — so callers barely changed. Strings are turned into LOCAL Dates by hand (`parseFieldValue()`), never with `new Date(\'YYYY-MM-DD\')`, which is read as UTC and can shift the day. Used by campaign scheduling (`StepSettings.jsx`), the Reports export range and the Activity Log range.',
          'Month and weekday names come from the browser\'s `Intl` for the active language (`src/i18n/datepickerLocale.js`), not from date-fns locale files: date-fns has none for Malayalam, Marathi or Punjabi, and importing the rest would bloat the bundle. The typed/displayed format is deliberately numeric (`dd/MM/yyyy`, `dd/MM/yyyy HH:mm`, 24-hour) so typing a date works identically in every language.',
          'The calendar popup renders in a portal on `document.body` (so a `Sheet` or scroll container can\'t clip it) at `z-index: 2000`, and `preventOpenOnFocus` is set so a dialog that auto-focuses its first field doesn\'t pop the calendar open by itself. Styling lives in `src/styles/_datepicker.scss`: the vendor CSS assumes 1rem = 16px while this app uses 1rem = 10px, so sizes are rebased there, and every colour (header band, weekday names, today ring, selected day and time, hover, arrows, borders) comes from the theme tokens (`--mw-primary*`, `--mw-surface*`), so dark mode and each accent colour restyle the calendar automatically. A calendar icon sits at the input\'s end (`.mw-datefield__icon`, our own element rather than the library\'s `showIcon`, whose CSS would override the input padding); clicking it opens the calendar, and the clear (×) button of clearable fields sits just before it.',
        ],
        note: {
          tone: 'info',
          icon: 'bi-info-circle',
          text: 'Need a new date field? Use `<DateField>` — do not add a native date input. It also handles one interaction for you: `Sheet` listens for Escape on `document` (capture phase) and would close the whole dialog, so while a calendar is open `DateField` listens on `window` first, closes just the calendar and stops the event — the first Escape closes the calendar, the second closes the dialog.',
        },
      },
      {
    ...
    sections: [
      ...
      },
      {
        ...
        facts: [
          ...
          ['`middleware/actionLimiter.js`', 'Per-USER (not per-IP) `express-rate-limit` instances for a handful of heavy/sensitive actions — campaign create/send/schedule/resend/recipients, image upload, manual backup create/upload/restore. Layered on top of `app.js`\'s blanket 300/min-per-IP ceiling, which alone can\'t stop one compromised account from hammering an expensive action. Every limiter\'s `message` is a short, self-contained sentence — never a guessed wait time (e.g. never "wait 15 minutes") — because `src/api/client.js` already appends the REAL one, read from the `Retry-After` header `express-rate-limit` sets automatically on every 429 (in seconds). This requires `app.js`\'s `cors()` config to explicitly list `exposedHeaders: [\'Retry-After\']` — without it the header exists on the wire but browsers hide it from `fetch()`, silently. If you add a new limiter, give it a plain, time-free `message` and this appending happens for free everywhere that already does `toast.error(error.message)`.'],
    ...
    sections: [
      {
        heading: 'The wizard',
        paragraphs: [
          '`CampaignWizardPage.jsx` drives a 6-step flow (`components/wizard/`: Info, Recipients, Template, Content, Settings, Review). The final step offers three outcomes, all going through `createOrUpdateCampaign(sendAt)` (`POST/PUT /api/campaigns`, then `POST /:id/recipients`) so campaign-creation logic lives in exactly one place: "Send campaign" (`startSending()`, sends or schedules immediately), "Save as Draft" (`saveDraft()`, `sendAt = null`, no send/schedule call at all — navigates straight to `/campaigns/:id` Analytics), or leaving mid-wizard (nothing persists until one of the two buttons is pressed). Editing an existing Draft (`/campaigns/:id/edit`) reuses the same wizard; its `PageHeader` gains a "View campaign analytics" link since `CampaignsPage.jsx` routes Draft rows to the wizard by default, not Analytics.',
          ...
          'Recipients step: `INITIAL_DRAFT.recipientSource` is deliberately `\'\'` — no card is pre-selected, and `stepError(1)` refuses Continue (and Send / Save as Draft, which re-run every step\'s check, and the Stepper header jumps) until a source is chosen AND it yields at least one address: a non-empty typed list, a group/segment with a count above zero, ticked subscribers, or a filter that matches someone. Historically the default was `\'all\'`, which none of the cards represented — so clicking Continue without touching anything quietly emailed every contact. The single exemption is editing a Draft that already has recipients (`originalRecipientCount > 0`): its source can\'t be restored and `createOrUpdateCampaign()` never re-adds recipients in that case. `recipientPayload()` now throws instead of falling back to `\'all\'`.',
          'Personalisation data: `POST /api/campaigns/:id/recipients` passes `source: \'list\'` (typed addresses AND segments — the wizard sends a segment\'s members as a plain list) and `source: \'subscribers\'` through `fillFromContacts()` in `routes/campaigns.js`. It looks each address up in `contacts` (case-insensitive) and fills `name`/`company`/`phone`/`city` wherever the caller didn\'t supply a value — a value typed in the wizard always wins, and an address that isn\'t a contact is left alone. Without this `{{name}}`, `{{company}}`, `{{phone}}` and `{{city}}` came out blank for those sources (only group/filter/all read the contacts table directly, and `subscribers` has no `phone` column at all). Typed lines may also be `Name <email@example.com>` — `parseManualList()` splits the name out.',
          'Subscribe button: `StepSettings.jsx` (and the Review row) hide the "Add a Subscribe button" switch when the frozen `draft.templateHtml` already contains `{{subscribe_url}}` (`utils/templateTokens.js` → `templateHasSubscribeUrl()`). Always test the RESOLVED html, not the raw template: `resolveTemplateFieldTokens()` turns a design-schema button URL into `{{subscribe_url}}` at the moment a template is picked, so many default templates only contain it after selection. A wizard effect switches `draft.subscribeButton` back off in that case, and `services/render.js`\'s `buildEmail()` independently skips the footer Subscribe button when the merged html already carries this recipient\'s subscribe link (checked before click-tracking rewrites links) — so even an old draft can\'t get two Subscribe buttons.',
          'Subject chips (`StepContent.jsx`) insert `{{field}}` at the caret — remembered on the input\'s `select`/`blur` because pressing a chip blurs the input — replacing any selected text, and append at the end only if the input was never focused.',
          'Campaigns list (`CampaignsPage.jsx`) uses the same bulk-selection pieces as Contacts (`useBulkSelection`, `BulkBar`): the header tick = the current page, "Select all N" fetches every matching id from `GET /api/campaigns/ids` (built by the same `buildCampaignListFilter()` as the list itself, capped at 5,000), and Delete calls `POST /api/campaigns/bulk-delete` after a confirm sheet. Running campaigns (`Sending`/`Testing`/`Sending Winner`) are refused by the DELETE statement\'s own `WHERE status NOT IN (...)` — no read-then-delete race — and come back in the response as `skipped`, same rule as the single-delete route.',
        ],
      },
      {
        heading: 'Lifecycle',
      ...
      },
      {
        ...
        paragraphs: [
          ...
          'Frontend: the whole A/B UI lives in `src/components/campaigns/ABTestPanel.jsx`, rendered UNCONDITIONALLY from `CampaignAnalyticsPage.jsx` — deliberately not a step in `CampaignWizardPage.jsx`, to avoid touching that wizard\'s step-index logic. A campaign can only turn A/B on while it\'s still `Draft`/`Scheduled` and hasn\'t started its test (`ab_test_started_at IS NULL`); once started, settings and variants lock. `ABTestPanel` decides for itself what to render (or nothing) for every state — the enable-prompt for a not-yet-enabled Draft, the config form, the live Testing dashboard, the winner banner, etc.',
          'Gotcha (real bug, already fixed, worth knowing so it doesn\'t come back): `CampaignAnalyticsPage.jsx` used to gate the render with `{campaign.ab?.enabled ? <ABTestPanel ... /> : null}`. That looks reasonable but is wrong — the "Set up an A/B test" button that turns A/B on in the first place lives INSIDE `ABTestPanel`, for the `!ab.enabled` case. Gating the whole component behind `ab.enabled` meant that button could never be reached by anyone, ever — a permanent chicken-and-egg lockout. `ABTestPanel` must always be rendered unconditionally on this page; let the component\'s own internal branching (which already handles every state, including "nothing to show") decide what appears.',
          'Second gotcha from the same investigation: the wizard (`CampaignWizardPage.jsx`) originally had no way to create a campaign as a plain, unsent `Draft` — the only final action was "Send campaign" (`startSending()`, which always either sends immediately or schedules). Since A/B setup requires a `Draft`/`Scheduled` campaign that hasn\'t sent, this made A/B unreachable through the normal flow too. Fixed by extracting the shared "create campaign + add recipients" logic into `createOrUpdateCampaign(sendAt)`, and adding a `saveDraft()` path (wired to a "Save as Draft" button next to Send) that calls it with `sendAt = null` and skips the send/schedule step entirely, then navigates straight to `/campaigns/:id` (Analytics). The wizard\'s edit view (`isEditing`) also gained a "View campaign analytics" link in its `PageHeader` — editing a Draft has no other visible path back to Analytics, since `CampaignsPage.jsx` routes Draft rows there by default rather than to Analytics.',
        ],
      },
      {
        heading: 'Important files',
        fileCards: [
          {
            ...
            does: 'Campaign CRUD, the wizard\'s save endpoint, schedule/pause/resume, the `.xlsx` report download, the list tick-box helpers (`GET /ids`, `POST /bulk-delete`), contact-data fill-in for typed/segment/subscriber recipients (`fillFromContacts()`), and the `/:id/ab/*` routes (settings, variants, stats, start, decide-winner, send-winner, cancel).',
    ...
    sections: [
      {
        ...
        facts: [
          ...
          ['Date / time picker (look or behavior)', '`src/components/ui/DateField.jsx` (the component), `src/i18n/datepickerLocale.js` (month/day names) and `src/styles/_datepicker.scss` (colors and sizes) — see Chapter 2'],
          ['Which recipient rules the campaign wizard enforces', '`stepError()` in `src/pages/CampaignWizardPage.jsx` (per-step checks) and `fillFromContacts()` in `server/src/routes/campaigns.js` (filling name/company/phone/city from Contacts) — see Chapter 11'],
