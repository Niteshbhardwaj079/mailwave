    ...
    sections: [
      ...
      },
      {
        ...
        paragraphs: [
          ...
          'The calendar popup renders in a portal on `document.body` (so a `Sheet` or scroll container can\'t clip it) at `z-index: 2000`, and `preventOpenOnFocus` is set so a dialog that auto-focuses its first field doesn\'t pop the calendar open by itself. Styling lives in `src/styles/_datepicker.scss`: the vendor CSS assumes 1rem = 16px while this app uses 1rem = 10px, so sizes are rebased there, and colours come from the theme tokens so dark mode and accent colours work automatically.',
        ],
        note: {
          tone: 'info',
          icon: 'bi-info-circle',
